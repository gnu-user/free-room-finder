...
/*  View 2: average number of people using the room on a day 
    not at the start of any class */
SELECT
    AVG(oc.num_people)
FROM
    occupied AS oc,
...
/*  View 3: Returns all course names that start at specified time (where that 
    time is whether any 2 courses from the same faculty share the same start 
    time regardless of day) */
SELECT
    c.name
FROM
    courses AS c,
...
/*  View 4: SHOULD SHOW ALL THE CLASSES and rooms (even the class that have 
    no room and the room does not have a class) */
SELECT
    *
FROM
    professors AS p,
...
/*  View 5: get all the prof's that are teaching course for both the MATH 
    faculty and the ENGR faculty */
(SELECT
    p.name
FROM
    professors AS p,
...
/*  View 6: Get enties that start on a monday (M) */
SELECT
    *
FROM
    offerings AS o,
