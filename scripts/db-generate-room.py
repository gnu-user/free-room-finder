...
def parse_course_info(course_content, course_data):
    ...
    for course_table in course_content.findAll('table', {'class': "datadisplaytable", 'summary': 
        ...
        # Finds all instances of the prof name
        idx_key = 0
        for row in course_table.findAll('span', {'classreverse_lookup(h, 2)': "fieldlabeltext"}, True, re.compile(r'Instructors:\s')):
            match = re_prof_name.search(str(row.next).strip())
            if match is not None:
                #print "teacher name", idx_key
...
def parse_class_time(course_content, course_data):
    ...
    for meeting_table in course_content.findAll('table', {'class': "bordertable", 'summary': 
        ...
        for row in meeting_table.findAll('tr'):
            ...
            for column in row.findAll('td', {'class': "dbdefault"}, True, re_week_info):
                print "COLUMN: ", column.strip()
                
                #if column == '&nbsp;':
                if re.match(re.compile(r'^&nbsp;$'), column.strip()):
                    print "MATCH 1"
                    course_data[idx_key][alp_key] = {'week1': False}
                    course_data[idx_key][alp_key]['week2'] = False
                elif re.match(re.compile(r'^(&nbsp;)*W1$'), column.strip()):
                #elif column == 'W1':
                    print "MATCH2"
                    course_data[idx_key][alp_key] = {'week1': True}
                    course_data[idx_key][alp_key]['week2'] = False
                elif re.match(re.compile(r'^(&nbsp;)*W2$'), column.strip()):
                #elif column == 'W2':
                    print "MATCH3"
                    course_data[idx_key][alp_key] = {'week1': False}
                    course_data[idx_key][alp_key]['week2'] = True
                    
            # Parse the course start and end time
            for column in row.findAll('td', {'class': "dbdefault"}, True, re_course_time):
            ...
            for column in row.findAll('td', {'class': "dbdefault"}, True, re_course_day):
            ...
            for column in row.findAll('td', {'class': "dbdefault"}, True, re_room):
            ...
            for column in row.findAll('td', {'class': "dbdefault"}, True, re_course_date):
            ...
            for column in row.findAll('td', {'class': "dbdefault"}, True, re_class_type):
